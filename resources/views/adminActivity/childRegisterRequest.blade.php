                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full table-auto border-collapse border border-gray-300">
                        ...
                        <tbody id="registrationTable">
                            @foreach($parentRecords as $index => $record)
                            @php
                                $enrollment = $record->enrollment;
                            @endphp
                            <tr class="border-b hover:bg-gray-50">
                                <!-- Index Number -->
                                <td>{{ $parentRecords->firstItem() + $index }}</td>
                                ...
                                <!-- Children Names -->
                                <td class="border px-4 py-2">
                                    {{ $record->child ? $record->child->child_name : 'No children registered' }}
                                </td>
                                ...
                                <!-- Details -->
                                <td class="border px-4 py-2">
                                    <a href="{{ route('adminActivity.show', ['adminActivity' => $record->id]) }}" class="text-blue-500 hover:text-blue-700 underline">View Details</a>
                                </td>
                                ...
                                <td>
                                    <div class="mt-1 flex justify-center gap-4">
                                        ...
                                        <button onclick="openModal('approve',  {{ $record->id }},
                                            '{{ $record->father->father_email ?? '' }}',
                                            '{{ $record->mother->mother_email ?? '' }}',
                                            '{{ $record->guardian->guardian_email ?? '' }}',
                                            '{{ $enrollment->registration_type ?? '' }}',
                                            '{{ $record->father->father_name ?? '' }}',
                                            '{{ $record->mother->mother_name ?? '' }}',
                                            '{{ $record->guardian->guardian_name ?? '' }}',
                                            '{{ $record->father->father_ic ?? '' }}',
                                            '{{ $record->mother->mother_ic ?? '' }}',
                                            '{{ $record->guardian->guardian_ic ?? '' }}'
                                        )"
                                        ...
                                        <form action="{{ route('adminActivity.destroy', $record->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this enrollment?');">
                                            @csrf
                                            @method('DELETE')
